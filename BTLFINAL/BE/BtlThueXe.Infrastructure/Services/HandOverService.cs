...
public class HandOverService : IHandOverService
{
        ...
        int currentUserId)
    {
        ...
        try
        {
            ...
            // =================================================
            // NHÂN VIÊN ĐÃ GIAO -> CHỜ KHÁCH XÁC NHẬN NHẬN XE
            // Chưa chuyển xe sang RENTING cho đến khi khách xác nhận.
            // =================================================
            string? oldContractStatus = hopDong.TrangThai;
            hopDong.TrangThai = "WAITING_CUSTOMER_RECEIVE";

            _context.LichSuTrangThaiHopDongs.Add(
                new LichSuTrangThaiHopDong
                {
                    IdHopDong = hopDong.Id,
                    TrangThaiCu = oldContractStatus,
                    TrangThaiMoi = "WAITING_CUSTOMER_RECEIVE",
                    IdNguoiThayDoi = currentUserId,
                    LyDo = "Nhân viên đã bàn giao xe, chờ khách hàng xác nhận nhận xe.",
                    ThoiGianThayDoi = now
                });

            string? oldVehicleStatus = xe.TrangThai;
            ...
            _context.AuditLogs.Add(new AuditLog
            {
                ...
                DuLieuMoi = "Contract=WAITING_CUSTOMER_RECEIVE; Vehicle=RESERVED",
                MoTa = $"Nhân viên đã giao xe #{xe.Id}, chờ khách xác nhận nhận xe cho hợp đồng #{hopDong.Id}.",
                ThoiGian = now
            });
    ...
    // =========================================================
    // GET BÀN GIAO THEO HỢP ĐỒNG CÓ KIỂM TRA QUYỀN
    // =========================================================
    public async Task<HandoverResponse?> GetByContractForUserAsync(
        int idHopDong, int currentUserId, bool isStaff)
    {
        ...
        if (!isStaff)
        {
            var customerUserId = handover.IdHopDongNavigation?
                .IdYeuCauThueNavigation?
                .IdKhachHangNavigation?
                .IdNguoiDung;

            if (customerUserId != currentUserId)
                throw new UnauthorizedAccessException("Bạn không có quyền xem thông tin bàn giao của hợp đồng này.");
        }
    ...
    // =========================================================
    // KHÁCH HÀNG XÁC NHẬN ĐÃ NHẬN XE
    // WAITING_CUSTOMER_RECEIVE -> IN_PROGRESS
    // RESERVED -> RENTING
    // =========================================================
    public async Task<HandoverResponse> ConfirmReceivedAsync(
        int idHopDong, int currentUserId)
    {
        var handover = await _context.BanGiaoXes
            .Include(x => x.IdHopDongNavigation)
                .ThenInclude(x => x.IdYeuCauThueNavigation)
                    .ThenInclude(x => x.IdKhachHangNavigation)
            .Include(x => x.IdXeNavigation)
            .FirstOrDefaultAsync(x => x.IdHopDong == idHopDong);

        if (handover == null)
            throw new KeyNotFoundException("Không tìm thấy thông tin bàn giao xe.");

        var hopDong = handover.IdHopDongNavigation;
        var yeuCau = hopDong.IdYeuCauThueNavigation;
        var khachHang = yeuCau?.IdKhachHangNavigation;

        if (khachHang == null || khachHang.IdNguoiDung != currentUserId)
            throw new UnauthorizedAccessException("Bạn không có quyền xác nhận nhận xe của hợp đồng này.");

        if (!string.Equals(hopDong.TrangThai, "WAITING_CUSTOMER_RECEIVE", StringComparison.OrdinalIgnoreCase))
            throw new InvalidOperationException($"Hợp đồng đang ở trạng thái {hopDong.TrangThai}, không thể xác nhận nhận xe.");

        var xe = handover.IdXeNavigation;
        if (!string.Equals(xe.TrangThai, "RESERVED", StringComparison.OrdinalIgnoreCase))
            throw new InvalidOperationException($"Xe đang ở trạng thái {xe.TrangThai}, không thể bắt đầu thuê.");

        var now = DateTime.UtcNow;
        await using var transaction = await _context.Database.BeginTransactionAsync();
        try
        {
            hopDong.TrangThai = "IN_PROGRESS";
            hopDong.ThoiGianNhanThucTe = now;
            xe.TrangThai = "RENTING";

            _context.LichSuTrangThaiHopDongs.Add(new LichSuTrangThaiHopDong
            {
                IdHopDong = hopDong.Id,
                TrangThaiCu = "WAITING_CUSTOMER_RECEIVE",
                TrangThaiMoi = "IN_PROGRESS",
                IdNguoiThayDoi = currentUserId,
                LyDo = "Khách hàng xác nhận đã nhận xe.",
                ThoiGianThayDoi = now
            });

            _context.LichSuTrangThaiXes.Add(new LichSuTrangThaiXe
            {
                IdXe = xe.Id,
                TrangThaiCu = "RESERVED",
                TrangThaiMoi = "RENTING",
                IdNguoiThayDoi = currentUserId,
                LyDo = $"Khách xác nhận nhận xe cho hợp đồng #{hopDong.Id}.",
                ThoiGianThayDoi = now
            });
            ...
            {
                IdNguoiDung = currentUserId,
                HanhDong = "CUSTOMER_CONFIRM_RECEIVE_VEHICLE",
                LoaiDoiTuong = "HOP_DONG",
                IdDoiTuong = hopDong.Id,
                DuLieuCu = "Contract=WAITING_CUSTOMER_RECEIVE; Vehicle=RESERVED",
                DuLieuMoi = "Contract=IN_PROGRESS; Vehicle=RENTING",
                MoTa = $"Khách hàng xác nhận đã nhận xe #{xe.Id}.",
                ThoiGian = now
            });
