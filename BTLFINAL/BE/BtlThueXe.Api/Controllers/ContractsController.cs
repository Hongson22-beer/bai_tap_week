...
namespace BtlThueXe.Api.Controllers
{
    ...
    public class ContractsController : ControllerBase
    {
        ...
        #region 1B. GET /api/contracts

        [HttpGet]
        [Authorize(Roles = "KHACH_HANG,NHAN_VIEN,ADMIN")]
        public async Task<ActionResult<List<ContractDetailDto>>> GetContracts()
        {
            int currentUserId = GetCurrentUserId();
            string? currentUserRole = GetCurrentUserRole();
            var result = await _contractService.GetContractsAsync(currentUserId, currentUserRole);
            return Ok(result);
        }
