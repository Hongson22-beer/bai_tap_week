...
public class VehiclesController : ControllerBase
{
    ...
    {
        if (file == null || file.Length == 0) return BadRequest(new { message = "Vui lòng chọn ảnh xe." });
        var ext = Path.GetExtension(file.FileName).ToLowerInvariant();
        if (!new[] { ".jpg", ".jpeg", ".png", ".webp" }.Contains(ext))
            return BadRequest(new { message = "Chỉ hỗ trợ JPG, JPEG, PNG hoặc WEBP." });
        if (file.Length > 5 * 1024 * 1024) return BadRequest(new { message = "Ảnh xe tối đa 5 MB." });

        var root = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "uploads", "vehicles");
        Directory.CreateDirectory(root);
        var fileName = $"vehicle-{id}-{Guid.NewGuid():N}{ext}";
        var fullPath = Path.Combine(root, fileName);
        await using (var stream = System.IO.File.Create(fullPath)) await file.CopyToAsync(stream);
        var publicPath = $"/uploads/vehicles/{fileName}";
        var vehicle = await _vehicleService.SetImageAsync(id, publicPath);
        return Ok(vehicle);
    }
}
