...
ValidateRole(currentUserRole, "NHAN_VIEN");
        ...
        #region 2B. GetContractsAsync

        public async Task<List<ContractDetailDto>> GetContractsAsync(
            int currentUserId,
            string? currentUserRole)
        {
            var query = _context.HopDongs.AsNoTracking().AsQueryable();
            ...
            {
                ValidateRole(currentUserRole, "NHAN_VIEN", "ADMIN");
            }
            ...
            return items.Select(MapToDetailDto).ToList();
        }
